        ...
        .map(|variant| {
            ...
            let msg_construction = match &variant.fields {
                syn::Fields::Unnamed(fields) => {
                    let field_from_arg_mappers = fields
                        .unnamed
                        .iter()
                        .enumerate()
                        .map(|(i, field)| {
                                    ...
                                    "event" | "variant" => {
                                        ...
                                        if let ConversionType::NoConversion = conversion {
                                            ...
                                            return Err(Error::new_spanned(
                                                value,
                                                format!("already converting to {}", conversion.name()),
                                            ));
                                        }
                                    }
                                    _ => {
                            ...
                            let type_error = syn::LitStr::new(
                                &format!("Wrong type for paramter {} of {}", i, variant_ident),
                                field.ty.span(),
                            );
                            let none_error =
                                syn::LitStr::new(&format!("Paramter {} of {} is None", i, variant_ident), field.ty.span());
                ...
                syn::Fields::Named(_) => {
                    return Err(Error::new_spanned(
                        variant,
                        "BuilderSignal only supports unit or tuple variants (even if they are empty)",
                    ))
                }
            };
            Ok((
